...
function bump() {
  local current=$1
  local delta=$2
  if [[ $current =~ ^(v?)(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)(-((0|[1-9][0-9]*|[0-9]*[a-zA-Z-][0-9a-zA-Z-]*)(\.(0|[1-9][0-9]*|[0-9]*[a-zA-Z-][0-9a-zA-Z-]*))*))?(\+([0-9a-zA-Z-]+(\.[0-9a-zA-Z-]+)*))?$ ]]; then
	local v=${BASH_REMATCH[1]}
	local major=${BASH_REMATCH[2]}
	local minor=${BASH_REMATCH[3]}
	local patch=${BASH_REMATCH[4]}
	local pre=${BASH_REMATCH[6]}
	local build=${BASH_REMATCH[11]}
	if [[ $delta =~ ^(major|minor|patch|((v?)(\*|[-\+]|([-\+]?(0|[1-9][0-9]*)))\.(\*|[-\+]|([-\+]?(0|[1-9][0-9]*)))\.(\*|[-\+]|([-\+]?(0|[1-9][0-9]*)))))(-(\*|((0|[1-9][0-9]*|[0-9]*[a-zA-Z-][0-9a-zA-Z-]*)(\.(0|[1-9][0-9]*|[0-9]*[a-zA-Z-][0-9a-zA-Z-]*))*)))?(\+(\*|([0-9a-zA-Z-]+(\.[0-9a-zA-Z-]+)*)))?$ ]]; then
	  local keyword=${BASH_REMATCH[1]}
	  local dmajor=${BASH_REMATCH[4]}
	  local dminor=${BASH_REMATCH[7]}
	  local dpatch=${BASH_REMATCH[10]}
	  local dpre=${BASH_REMATCH[14]}
